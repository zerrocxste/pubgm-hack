...
void CDrawing::AddCircle3D(Matrix4x4 matrix, Vector position, float points, float radius, ImColor& color)
{
	...
	for (float a = 0; a < (IM_PI * 2.0f); a += step)
	{
		...
		if (!utils::world_to_screen(matrix, start, start2d) || !utils::world_to_screen(matrix, end, end2d))
			return;
